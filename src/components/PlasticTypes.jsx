...
// src/components/PlasticTypes.jsx
import React, { useState, useEffect } from "react";
import { useNavigate, useLocation } from "react-router-dom";
import { fetchPlastics } from "../lib/api";   // 👈 import API helper
...
export default function PlasticTypes() {
  ...
  useEffect(() => {
    setLoading(true);
    setError("");
    fetchPlastics(search)   // 👈 call API helper
      .then((data) => {
        setTypes(data);
        setLoading(false);
      })
          ...
          types.map((t) => {
            ...
            return (
              <div
                ...
                onClick={
                  t.code ? () => navigate(`/plastic-types/${t.code}`) : undefined
                }
                style={{
                  cursor: t.code ? "pointer" : "default",
                  minHeight: "260px",
                  display: "flex",
              ...
              >
                <div
                  className={`text-2xl font-extrabold mb-2 ${
                    isWikipedia ? "text-blue-700" : "text-green-700"
                  }`}
                >
                  {t.name}
                  {isWikipedia && (
                    ...
                    >
                      <svg
                        className="inline w-5 h-5 text-blue-400"
                        fill="currentColor"
                        viewBox="0 0 20 20"
                      >
                ...
                {isWikipedia && t.wikipediaUrl && (
                  <div className="mt-4">
                    ...
                    <div className="text-xs text-blue-500 mt-1">
                      This result is from Wikipedia and may not be specific to
                      plastic types.
                    </div>
                  </div>
                )}
